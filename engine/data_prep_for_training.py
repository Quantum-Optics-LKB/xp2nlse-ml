...
def data_split(
    ...
    across all provided arrays (E, n2_values, power_values, power_values) to maintain consistency 
    between data points and their corresponding values.

    Parameters:
    - E (np.ndarray): The dataset array containing the features, expected to be of shape 
      [num_samples, num_channels, height, width].
    - n2_values (np.ndarray): Array of n2 values for each data sample.
    - isat_values (np.ndarray): Array of isat values for each data sample.
    ...
    (train, validation, test). Each of these tuples contain the split's data (E), n2 values, 
    power values (power_values), and power values (power_values), in that order.

    Example Usage:
        (train_data, train_n2, train_power_label, train_power_value), 
        (val_data, val_n2, val_power_label, val_power_value), 
        (test_data, test_n2, test_power_label, test_power_value) = data_split(E, n2_values, power_values, power_values)
    ...
    return (E[training_indices], n2_values[training_indices],isat_values[training_indices]), (E[validation_indices], n2_values[validation_indices],isat_values[validation_indices]),(E[test_indices], n2_values[test_indices],isat_values[test_indices])
...
def data_treatment(
    ...
    - myset (np.ndarray): The dataset containing the optical field data, structured as
      [num_samples, num_channels, height, width].
    - n2label (np.ndarray): The values for the nonlinear refractive index (n2), with shape
      [num_samples].
    - isatlabel (np.ndarray): The categorical values for the saturation intensity, intended for
    ...
    evaluation loop, providing batches of data and corresponding values.
