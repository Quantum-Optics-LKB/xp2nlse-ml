...
class Inception_ResNetv2(nn.Module):
    def __init__(self, in_channels=2, k=256, l=256, m=384, n=384):
        ...
        self.linear_1 = nn.Sequential(nn.Linear(1536, 1024), nn.ReLU(), nn.Dropout(p=0.35))
        self.linear_2 = nn.Sequential(nn.Linear(1024, 512), nn.ReLU(), nn.Dropout(p=0.35))
        self.linear_3 = nn.Sequential(nn.Linear(512, 256), nn.ReLU(), nn.Dropout(p=0.35))
        self.linear_4 = nn.Sequential(nn.Linear(256, 128), nn.ReLU(), nn.Dropout(p=0.35))
        self.linear_5 = nn.Sequential(nn.Linear(128, 64), nn.ReLU(), nn.Dropout(p=0.35))
        self.linear_6 = nn.Linear(64, 3)
